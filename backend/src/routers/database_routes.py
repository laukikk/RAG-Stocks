...
# Custom imports
from database.calls import DatabaseCalls
...
def create_user(username: str, email: str, password: str, 
    ...
    try:
        ...
        new_user = DatabaseCalls.create_user(
...
def get_user_by_email(email: str, db: Session = Depends(neon_client.get_db_session)):
    """
    Retrieve a user using their email address.
    """
    user = DatabaseCalls.get_user_by_email(db, email)
    if user:
        return user
...
def get_user_portfolio_summary(user_id: int, db: Session = Depends(neon_client.get_db_session)):
    """
    Get a detailed summary of a user's portfolio across all accounts.
    """
    portfolio = DatabaseCalls.get_user_portfolio_summary(db, user_id)
    if portfolio:
        return portfolio
...
def create_account(user_id: int, account_type: str, currency: str = 'USD', db: Session = Depends(neon_client.get_db_session)):
    """
    Create a new account for a user.
    """
    try:
        account = DatabaseCalls.create_account(db, user_id, account_type, currency)
        return account
    except Exception as e:
        raise HTTPException(status_code=500, detail=str(e))
...
def create_stock(symbol: str, company_name: str, exchange: str, 
    ...
    """
    try:
        stock = DatabaseCalls.create_stock(db, symbol, company_name, exchange, sector, industry)
        return stock
    except Exception as e:
        raise HTTPException(status_code=500, detail=str(e))
...
def record_stock_daily_price(stock_id: int, date: date, close_price: float, 
    ...
    """
    try:
        daily_price = DatabaseCalls.record_stock_daily_price(
...
def get_stock_price_history(stock_id: int, start_date: date = None, end_date: date = None, 
                            db: Session = Depends(neon_client.get_db_session)):
    """
    Retrieve the historical price data for a given stock.
    """
    history = DatabaseCalls.get_stock_price_history(db, stock_id, start_date, end_date)
    if history:
        return history
...
def get_stock_performance(stock_id: int, days: int = 30, 
                          db: Session = Depends(neon_client.get_db_session)):
    """
    Get performance metrics for a stock over a specified number of days.
    """
    performance = DatabaseCalls.get_stock_performance(db, stock_id, days)
    if performance:
        return performance
...
def add_portfolio_holding(account_id: int, stock_id: int, quantity: int, average_price: float, 
                          db: Session = Depends(neon_client.get_db_session)):
    """
    Add a stock holding to a user's portfolio.
    """
    try:
        holding = DatabaseCalls.add_portfolio_holding(db, account_id, stock_id, quantity, average_price)
        return holding
    except Exception as e:
        raise HTTPException(status_code=500, detail=str(e))
...
def create_order(account_id: int, stock_id: int, order_type: str, transaction_type: str, 
    ...
    """
    try:
        order = DatabaseCalls.create_order(
...
def record_transaction(account_id: int, stock_id: int, transaction_type: str, 
    ...
    """
    try:
        transaction = DatabaseCalls.record_transaction(
...
def create_watchlist(user_id: int, name: str, db: Session = Depends(neon_client.get_db_session)):
    """
    Create a new watchlist for a user.
    """
    try:
        watchlist = DatabaseCalls.create_watchlist(db, user_id, name)
        return watchlist
    except Exception as e:
        raise HTTPException(status_code=500, detail=str(e))
...
def add_watchlist_item(watchlist_id: int, stock_id: int, db: Session = Depends(neon_client.get_db_session)):
    """
    Add a stock to an existing watchlist.
    """
    try:
        item = DatabaseCalls.add_watchlist_item(db, watchlist_id, stock_id)
        return item
    except Exception as e:
        raise HTTPException(status_code=500, detail=str(e))
